...
void ImageProcessing(int _iCurrentImageNum) {
	std::lock_guard<std::mutex> ImageLock(mutex0);
	//Creating a bitmap handle for the bitmap, and loading it
	HBITMAP bitmapHandle = (HBITMAP)LoadImage( g_hInstance, g_vecImageFileNames[_iCurrentImageNum].c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	...
	HDC imageDC = CreateCompatibleDC(g_WindowDC);
	SelectObject(imageDC, bitmapHandle);

	//Printing the image to the backbuffer
	StretchBlt(g_BackBuffer.GetBufferDC(), 0 + iPrevX, 0 + iPrevY, 200, 200, imageDC, 0, 0, bitmap.bmWidth, bitmap.bmHeight, SRCCOPY);
	DeleteObject(imageDC);
	
	if ((_iCurrentImageNum + 1) % 6 == 0) {
		iPrevY += 200;
		iPrevX = 0;
	} else {
		iPrevX += 200;
...
void SoundProcessing(int _iCurrentSound) {

	//std::lock_guard<std::mutex> SoundLock(mutex1);
	if (g_vecSoundFileNames.empty()) return;

	//Formatting the file path 
	std::wstring OpenCommand = L"open \"" + g_vecSoundFileNames[_iCurrentSound] + L"\" type waveaudio alias sound";
	mciSendString(OpenCommand.c_str(), NULL, 0, 0);
	LPCWSTR PlayCommand = L"play sound";
...
LRESULT CALLBACK WindowProc(HWND _hwnd, UINT _uiMsg, WPARAM _wparam, LPARAM _lparam)
{
	...
	case WM_CREATE:
	{
			g_BackBuffer.ClearBuffer();
			g_BackBuffer.Initialize(_hwnd, _kuiWINDOWWIDTH, _kuiWINDOWHEIGHT);
			g_BackBuffer.ClearBuffer();
	}
	...
	case WM_COMMAND:
	{
		...
		case ID_FILE_LOADIMAGE:
		{
			if (ChooseImageFilesToLoad(_hwnd))
			{
				...
				//Variables for wrapping and positioning and whatnot. NEED TO BE LOCKED
				

				std::vector<std::thread> ImageThreadVect;
				for (int i = 0; i < g_vecImageFileNames.size(); i++) {
					ImageThreadVect.push_back(std::thread(ImageProcessing, i));
				}
				std::for_each(ImageThreadVect.begin(), ImageThreadVect.end(), std::mem_fn(&std::thread::join));
		...
		case ID_FILE_LOADSOUND:
		{
			if (ChooseSoundFilesToLoad(_hwnd))
			{
				...
				std::vector<std::thread> SoundThreadVect;
				for (int i = 0; i < g_vecSoundFileNames.size(); ++i) {
					SoundThreadVect.push_back(std::thread(SoundProcessing, i));
					Sleep(100);
				}
				//Joining all the threads back
				std::for_each(SoundThreadVect.begin(), SoundThreadVect.end(), std::mem_fn(&std::thread::join));
			}
			else
			{
